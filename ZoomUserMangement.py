...
apiURL = {
        ...
        'groupLockSettings':'v2/groups/@/lock_settings',
        'acctRecordings':'v2/accounts/@/recordings',
        'userRecordings':'v2/users/@/recordings',
...
def compareGroupSave(group1Name, group2Name, group1,group2):
    """Saves contents of two compared group settings that are not the same for
        use in file comparison tools like PSPAD
    ...
    """        
    try:
        with open(f'Settings-{group1Name}.JSON', 'w') as f:
            f.write(group1)
    except Exception as e:
        PrintException(e)
    try:
        with open(f'Settings-{group2Name}.JSON', 'w') as f:
            f.write(group2)
    except Exception as e:
        PrintException(e)
...
def ldapConnect():
    ...
    def ldapGet():
        """Retrieves provisioning resource (Active Directory via LDAP interface)
        Returns:
...
def download_file(url, fileExtension, mtype = "", user = "", topic = "", topicId = "", datePath = "", desc = ""):
    ...
    if is_admin():
        ...
        try:
            ...
            logging(f"Attempted to create directory {path}")
        except Exception as e:
...
def download_user_recordings():
    global userRec
    global userEmail
    global cancelAction
    global eTxtUserLogStart
    global eTxtUserLogEnd
    ...
    logging(f"Checking if {userEmail}/{userID} has any existing recordings where they were a host.")  
    
    startDate = None
    if userLogStart.get() != "" and eTxtUserLogStart != None:
        startDate = userLogStart.get()
    
    endDate = None
    if userLogEnd.get() != "" and eTxtUserLogEnd != None:
        endDate = userLogEnd.get()  
    

    get_user_recordings(userID, init = 1, dateStart = startDate, dateEnd = endDate)
    ...
    for mtg in userRec:
        for fileURL in userRec[mtg]["download"]:
            totalFiles += 1
            
    progress = stdProgressBarStyle(frameStatus, colorbar = 'black')
    ...
    for mtg in userRec:        
        topic = userRec[mtg]["topic"]
        timeStart = userRec[mtg]["timeStart"]
        notFound = False
        logging(f"Downloading {totalFiles} files.")    
        if cancelActionsCheck():
            return
   
        for fileURL in userRec[mtg]["download"]:
            counter += 1
            bar = int((counter/totalFiles)*100)
            progress_var.set(bar)
            
            if cancelActionsCheck():
                return
            idx = userRec[mtg]["download"].index(fileURL)
            fileExtenson = userRec[mtg]["fileExtension"][idx]
    
            
            download_file(
                fileURL,
                fileExtension = fileExtenson,
                mtype = "recording",
                user = userEmail,
                topic = topic,
                topicId = mtg,
    ...
    if notFound:
        logging(f"{userEmail} does not have any existing recordings where they were a host.")  
...
def get_recording_dates():
    ...
    logging(f"Checking with base date of: {calDate}")
    return calDate



def retrieveRecordingData(userID, dateStart, dateEnd, next_page_token = None):
    ...
    #1.  Get all records of recordings on account based on time period
    record = None
    
    try:
        record = send_REST_request('userRecordings', data=userID, param=query, rType = "get", leaseTime = 6)
        recordsTotal = record["total_records"]
        
        
        #try again, is this necessary?
        #if recordsTotal == 0:
        #    send_REST_request('userRecordings', data=userID, param=query, rType = "get", leaseTime = 6)
        #    recordsTotal = record["total_records"]
    ...
    return (record, recordsTotal)

def cancelActionsCheck():
    global cancelAction
...
def get_user_recordings(userID, init = 0, months = None, dateStart = None, dateEnd = None, recordsCount = 0, next_page_token = None, total = None, recData = {}):
    '''
    Retrieve all user recording IDs from account
    ...
    #Initialize record search on first recursion
    if init == 1:
        init = 0
        cancelActions(False)
        try:
            months = int(eRecMon.get())
        except:
            months = 0
    ...
    if dateStart is None:     
        ...
        #Check if end date exceeds today's date
        if  datetime.datetime.strptime(dateEnd, '%Y-%m-%d') > datetime.datetime.now():
            logging(f"Changing end date to today.")
            dateEnd = datetime.datetime.now().strftime("%Y-%m-%d")
    else:
        logging(f"Start Date set to: {dateStart}.")    
    ...
    (record, recordsTotal) = retrieveRecordingData(userID, dateStart, dateEnd, next_page_token)
        
    if record is None:
        logging(f"No recording metadata available for pass #{recordsCount}, starting next pass")
    else:
        #How many passes remain?
        try:
            steps = ((int(eRecMon.get())) - months) + 1
        except:
            steps = 1
            
        logging(f"Starting recording metadata scan, step {steps}/{(int(eRecMon.get()) * 1)}")        
        next_page_token = record["next_page_token"]
        
        calDate = datetime.datetime.strptime(dateStart, '%Y-%m-%d')
        logging(f"Reading Cloud Recording metadata for {calendar.month_name[calDate.month]}-{calDate.year}: ({recordsCount}/{recordsTotal})")
            
        #2.  Check each record returned and save meeting ID and separately store recording IDs
        for item in record["meetings"]:
            ...
            try:
                userRec[mtgId]["share"] = item["share_url"]
            except:
                userRec[mtgId]["share"] = ""
            
            for rec in item["recording_files"]: 
                userRec[mtgId]["recId"].append(rec["id"])
                try:
                    userRec[mtgId]["mtgId"].append(rec["meeting_id"])
                except:
                    userRec[mtgId]["mtgId"].append(mtgId)  
                try:
                    userRec[mtgId]["download"].append(rec["download_url"])
                except:
                    userRec[mtgId]["download"].append("")
                try:
                    userRec[mtgId]["fileType"].append(rec["file_type"])
                    if rec["file_type"] == 'TRANSCRIPT':
                        userRec[mtgId]["fileExtension"].append("VTT")    
                    elif rec["file_type"] == 'CHAT':
                        userRec[mtgId]["fileExtension"].append("CHAT.TXT")
                    elif rec["file_type"] == 'CC':
                        userRec[mtgId]["fileExtension"].append("CC.VTT")    
                    elif rec["file_type"] == 'TIMELINE':
                        userRec[mtgId]["fileExtension"].append("JSON")
                    else:
                        userRec[mtgId]["fileExtension"].append(rec["file_type"])
                    
                except:
                    userRec[mtgId]["fileType"].append("")
                try:
                    userRec[mtgId]["fileSize"].append(rec["file_size"])
                except:
                    userRec[mtgId]["fileSize"].append(0)    

    #Tally number of recorded meetings pulled
    try:
        recordsCount += len(record["meetings"])
    except:
        # recordsCount remains the same
        pass
        
    #Check if all records from all months in the range have been validated
    if months == None or months < 1:
        logging(f"Recording metadata scan completed: {recordsTotal}")
        btnDownload["state"] = "normal"
        return

    try:    
        bar = int((recordsCount/recordsTotal)*100)
        progress = stdProgressBarStyle(frameStatus, colorbar = 'blue')
        #progress_var.configure("red.Horizontal.TProgressbar", troughcolor ='blue', background='gray')
        progress_var.set(bar)
        
        #Check performance of this
        root.update()
    except Exception as e:
        bar = int(100-((months/int(eRecMon.get())*100)))
        progress = stdProgressBarStyle(frameStatus, colorbar = 'red')
        #progress_var.configure("red.Horizontal.TProgressbar", troughcolor ='red', background='gray')
        progress_var.set(months)
        root.update()
        #logging(f"Progress bar error: {e}")    
    
    
    #Check if all pages for current date range have been pulled
    # else check the next date range
    if next_page_token != "":
        get_user_recordings(
            userID,
            init = 1,
        ...
        months = months - 1
        monthUpdate = calDate.month - 1 if calDate.month > 1 else 12
        
        print (f"Month current:  {calDate.month} to {monthUpdate}")
        calDate = calDate.replace(month=monthUpdate)
        print (f"Month updated to {calDate.month}")
        ...
        #logging(f" Starting Participant scan, step {steps}/{(int(eRecMonths.get()) * 1)}")
        #logging(f"Reading Meeting participant emails for {len(userRec)} meetings...")
        
        #If needed for the future to save a log of the participants in the recording.
        get_meeting_participants(userRec)
        save_user_rec_metadata(userID,dateStart,dateEnd)
        

        if months > 0:       
            print (f"Months To Check ({eRecMon.get()}): Month {months}, Next Batch:  From: {dateStart} To: {dateEnd}")
            get_user_recordings(
                userID,
                months = months,
                dateStart = dateStart,
...
def get_account_recordings(months = None, dateStart = None, dateEnd = None, cnt = 0, next_page_token = None, total = None, recData = {}):
   ...
   # "to": dateEnd
    ...
    if next_page_token != "" or cnt == 0:
        ...
        save_acct_rec_metadata(acctRec,dateStart,dateEnd)
        months = months - 1
        if months > 0:
            calDate = calDate.replace(
                year = calDate.year if calDate.month > 1 else calDate.year - 1,
    ...
    try:
        with open(f'{RECDATA_FILE}-{startDate} to {endDate}.csv', 'w', encoding="utf-8", newline='') as csvFile:
            ...
            for meetingID in recData:
                csvRow = {
                    "RecordingID": recData[meetingID]["recId"],
...
def save_acct_rec_metadata(recData,startDate,endDate):
    try:
        with open(f'{RECDATA_FILE}-{startDate} to {endDate}.csv', 'w', encoding="utf-8", newline='') as csvFile:
            writer = csv.DictWriter(csvFile, fieldnames = RECDATA_FIELDS)
            writer.writeheader()
...
def get_userID(userEmail):
    ...
    #Validate against domain stored in settings
    # This also the user to type a userID in the field
    if domain != None and domain != "":
        if not domain in userEmail:
            return userEmail
...
def get_UserInfo(user, userId = None):
...
def get_group_data():
    ...
    try:
        groups = send_REST_request('groups', data = acct_id(), rType = "get")
    except Exception as e:
        groups = None
        print(f'Exception:{e}')    
...
def xfer_group_settings():
    global cancelAction
    global groupDB
    global lockSettingsDB
    global customGroup1List
    global customGroup2List
    global emenuGroup1
    global emenuGroup2
    global progress
    ...
    logging(f'Attempting transfer of settings')
    groupsData = get_group_data()

    sourceGroup = emenuGroup1.get()
    xferGroup = emenuGroup2.get()
    
    logging(f'Source Group: {sourceGroup}')
    logging(f'Destination Group: {xferGroup}')


    sourceSettings = groupDB[sourceGroup]
    sourceLockSettings = lockSettingsDB[sourceGroup]
    
    xferSettings = groupDB[xferGroup]
    xferLockSettings = lockSettingsDB[xferGroup]
    
    for gId in groupsData:
        if xferGroup in groupsData[gId]:
            xferGroupId = gId
    ...
    for category in sourceSettings:
        recordsCount += 1
        recordsSubCount = 0
        bar = int(recordsCount/len(sourceSettings)*100)
        progress = stdProgressBarStyle(frameStatus, colorbar = 'blue')
        #progress_var.configure("red.Horizontal.TProgressbar", troughcolor ='blue', background='gray')
        progress_var.set(bar)        
        #Check performance of this
        root.update()
        for setting in sourceSettings[category]:
            logging (f'Transferring {category}: {setting} to {xferGroup}')
            try:
                if setting == "meeting_authentication" or setting == "authentication_options":
                    apiParam ={
                        'option':'meeting_authentication'
                     }
                elif setting == "recording_authentication":
                   apiParam ={
                ...
                send_REST_request(
                    ...
                    data=xferGroupId,
                    param = apiParam,
                    body = {category:{setting:sourceSettings[category][setting]}},
                    rType = "patch"
                )
            except Exception as e:
                logging (f'Error in Updating {xferGroup} group {category} settings: {e}')
                
            try:
                if category == 'meeting_security' or setting == 'meeting_security':
                    apiParam1 = {
                    'option':'meeting_security'
                    }
                else:
                ...
                send_REST_request(
                    ...
                    data=xferGroupId,
                    param = apiParam1,
                    body= {category:{setting:sourceLockSettings[category][setting]}},
                    rType = "patch"
                ) 
            except Exception as e:
                logging (f'Error in Updating {xferGroup} group {category} lock settings: {e}')        
            
    compare_groups(sourceGroup,xferGroup)
...
def compare_groups(group1, group2):
    ...
    group1Data = json.dumps(groupDB[group1], sort_keys=True, indent=2)
    group2Data = json.dumps(groupDB[group2], sort_keys=True, indent=2)
    
    lock1Data = json.dumps(lockSettingsDB[group1], sort_keys=True, indent=2)
    lock2Data = json.dumps(lockSettingsDB[group2], sort_keys=True, indent=2)
    
    groupd1Data = f"{group1Data}\n{lock1Data}"
    groupd2Data = f"{group2Data}\n{lock2Data}"
    
    if group1Data == group2Data:
        logging (f'Group Settings for {group1} is the same as {group2}')
    else:
        logging (f'Group Settings for {group1} are not the same as {group2}')
        print(f'{group1Data}')
        print(f'{group2Data}')
        compareGroupSave(group1, group2, group1Data,group2Data)
                            ...
                            try:
                                for setting in groupSettings[category]:
                                    try:
                                        ...
                                        try:
                                            csvRow.update({"Locked":lockSetting[group][category][setting]})    
                                        except:
                                            pass
...
def get_groups_settings(groupData, groupsList = None, recordsCount = 0):
    global cancelAction
    global groupDB
    global lockSettingsDB
    global customGroup1List
    ...
    if acct_id() == 'me':
        groupName = 'Primary Account'
    else:
        groupName = acct_id()
        
    customGroup1List.append(groupName)
    customGroup2List.append(groupName)
    groupDB[groupName] = get_acct_settings()
    lockSettingsDB[groupName] = get_lock_settings()      
    
    
    for groupID in groupData:
        recordsCount += 1
        bar = int(recordsCount/len(groupData)*100)
        progress = stdProgressBarStyle(frameStatus, colorbar = 'red')
        progress_var.set(bar)
        root.update()  
        ...
        groupSettingsLocked = get_lock_settings(groupID)  
        groupSettings = get_group_settings(groupID, count)
        
        try:
            groupDB[group] = groupSettings
            lockSettingsDB[group] = groupSettingsLocked
            
            customGroup1List.append(group)
            customGroup2List.append(group)
            
        except Exception as e:
            PrintException(e)
     
    customGroup1.set(customGroup1List[0])
    emenuGroup1['values'] = customGroup1List

    customGroup2.set(customGroup2List[0])
    emenuGroup2['values'] = customGroup1List
    
    root.update() 
     
    save_acct_settings(groupDB, lockSettingsDB)   
...
def get_lock_settings(groupID = None):
    logging (f'Retrieving lock settings')
    try:
        acctID = acct_id()
        timeStart = time.time()
        ...
        if groupID == None:
            ...
            acctSettings = send_REST_request('groupLockSettings', data = groupID, rType = "get") 
        timeEnd = time.time()            
        timeTotal = timeEnd - timeStart
        #logging (f'Lock settings:{acctSettings}')     
    except Exception as e:
        PrintException(e)
...
def get_acct_settings(altID = None):
    acctSettings = {}
...
def stdProgressBarStyle(origin, length = 100, variable = None, colorbar = 'red'):
    s = ttk.Style()
    s.theme_use('clam')
    s.configure("red.Horizontal.TProgressbar", foreground= colorbar, background = colorbar)
...
btnRetrieveGroups = stdButtonStyle(frameSubMenuCntrl[1], text = "Retrieve All Group Data", width = 25, command = retrieveZoomGroups)
...
btnXferSettings = stdButtonStyle(\
    frameSubMenuCntrl[1],
    text = "Transfer Group Settings",
    width = 25,
    command = xfer_group_settings,
    state = DISABLED    
    )

stdButtonActionGrid(btnRetrieveGroups)
...
stdButtonActionGrid(btnXferSettings)
...
customGroup1 = StringVar()
customGroup1List = ["None"]
customGroup1.set(customGroup1List[0])

customGroup2 = StringVar()
customGroup2List = ["None"]
customGroup2.set(customGroup2List[0])

emenuGroup1 = ttk.Combobox(frameXferSettings, textvariable=customGroup1, values=customGroup1List)
elblAttribTo2 = stdLabelStyle(frameXferSettings, text=" to ")
emenuGroup2 = ttk.Combobox(frameXferSettings, textvariable=customGroup2, values=customGroup2List)

emenuGroup1.grid(row = rowPos, column = posC(1,colPos), sticky = E)
elblAttribTo2.grid(row = rowPos, column = posC(1,colPos), sticky = E)
emenuGroup2.grid(row = rowPos, column = posC(1,colPos), sticky = E)
...
eTxtUserLogStart = stdEntryStyle(txtUserLogFrame, textvariable= userLogStart, width=20)


eLblUserLogEnd = stdLabelStyle(txtUserLogFrame, text="Log End (yyyy-mm-dd)")
eTxtUserLogEnd = stdEntryStyle(txtUserLogFrame, textvariable = userLogEnd, width=20)


eLblRecMon = stdLabelStyle(txtUserLogFrame, text="No. of months to check for recordings")
eRecMon = stdEntryStyle(txtUserLogFrame)
...
eLblRecMon.grid(row = pos(1,rowPos), column = 0, columnspan=2, sticky = W)
eRecMon.grid(row = pos(1,rowPos), column = 1, columnspan=2, sticky = W)


eRecMon.delete(0, END)
eRecMon.insert(0, "6")
...
btnDownload2 = stdButtonStyle(frameSubMenuCntrl[2], text="User Recordings DL", command=download_user_recordings)
...
userEmailList = []
eLblUserEmail = stdLabelStyle(frameSubMenuCntrl[2], text="User Email / User ID")
...
stdButtonActionGrid(btnDownload2)
